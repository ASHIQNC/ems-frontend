...
import { singleEmployee, updateEmployee } from "../service/allApis";
...
const EditEmployee = () => {
  ...
  const handleEdit = async (e) => {
    e.preventDefault();
    const { email, fname, lname, gender, status, location, mobile, profile } =
      inputs;
    if (fname == "") {
      ...
      if (result.status >= 200 && result.status <= 300) {
        ...
        setUpdateData(result.data.fname);
        alert("data added");
        ...
        <form>
          ...
          <Col
            ...
            <img
              ...
              src={
                imgPreview ? imgPreview : `${BASE_URL}/upload/${inputs.profile}`
              }
              alt="profile"
            />
          </Col>
